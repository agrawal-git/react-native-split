...
var DEV_REGEX = /global\.__DEV__\s?=\s?true/;
var DEV_FALSE = 'global.__DEV__ = false';

function injectCodesToBase(config) {
  var entryInject = '\n\nrequire(\'AppRegistry\')\n';
  config.customEntries.forEach(function (entry) {
    if (entry.inject === false) {
      return;
    }
    var indexModule = path.resolve(config.root, entry.index);
    entryInject += 'require(\'' + indexModule + '\');\n';
  });
  var tmpEntry = path.resolve(config.root, config.baseEntry.index + '.tmp');
  if (fs.existsSync(tmpEntry)) {
    fs.unlinkSync(tmpEntry);
  }
  var originData = fs.readFileSync(config.baseEntry.index, 'utf-8');
...
function bundle(config, callback) {
  ...
  var tmpBase = injectCodesToBase(config);
  var bundlePath = path.resolve(config.bundleDir, 'index.bundle');

  var cmd = 'react-native bundle';
  ...
  exec(cmd, function (error) {
    ...
    var code = fs.readFileSync(bundlePath, 'utf-8');
    if (!config.dev) {
      var globalDev = DEV_REGEX.exec(code.substring(0, 5000));
      if (globalDev) {
